...
@registry.register_transform(
    display_name="ICT Brave AI Web Search",
    input_entity="ICT.brave.request",
    description="Brave AI Web Search (REST) con output strutturato.",
    output_entities=["ICT.Brave.type.web"],
)
class getBraveAIWebSearch(DiscoverableTransform):
    @classmethod
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        ...
        if max_results and max_results > 0:
            ...
            data = brave_request("/web/search", params, api_key_env=api_key_env)
            results = _extract_results(data, "web")
        log_message(f"Web results count: {len(results)}")
        for result in results:
            if isinstance(result, dict):
                _add_result_entity(response, result, str(query), "web", "ICT.Brave.type.web")
